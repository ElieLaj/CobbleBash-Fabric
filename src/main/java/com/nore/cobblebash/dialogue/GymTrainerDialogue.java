...
public class GymTrainerDialogue {
   private static final ResourceLocation DEFAULT_BACKGROUND = ResourceLocation.fromNamespaceAndPath("cobblemon", "textures/gui/dialogue/dialogue_box.png");
   ...
   public static boolean open(ServerPlayer player, RctApiProbe.GymTrainerRef trainerRef) {
      GymTrainerUnit gymtrainerunit = GymTrainerUnit.fromTrainerIdPart(trainerRef.trainerIdPart());
      if (gymtrainerunit == null) {
         return false;
      } else {
         RctTrainerDataLoader.TrainerData rcttrainerdataloader$trainerdata = RctTrainerDataLoader.load(
               player.server, trainerRef.gymType(), trainerRef.trainerIdPart()
            )
            .orElse(null);
         if (rcttrainerdataloader$trainerdata == null) {
            player.sendSystemMessage(Component.literal("No trainer JSON found for " + trainerRef.gymType() + " " + trainerRef.trainerIdPart() + "."));
            return true;
         } else {
            RctTrainerDataLoader.DialogueData rcttrainerdataloader$dialoguedata = rcttrainerdataloader$trainerdata.dialogue();
            DialogueManager.startDialogue(
               player, createDialogue(player, trainerRef, gymtrainerunit, rcttrainerdataloader$trainerdata, rcttrainerdataloader$dialoguedata)
            );
            return true;
         }
      }
   ...
   private static Dialogue createDialogue(
      ...
      DialogueAction dialogueaction = (activeDialogue, value) -> {
         activeDialogue.close();
         GymCommand.startTrainerBattle(player, trainerRef.gymType(), trainerRef.slotId(), unit);
      };
      DialogueAction dialogueaction1 = (activeDialogue, value) -> activeDialogue.close();
      DialogueOptionSetInput dialogueoptionsetinput = new DialogueOptionSetInput(
         List.of(
            new DialogueOption(text(dialogueData.battleText()), "battle", dialogueaction, ALWAYS_TRUE, ALWAYS_TRUE),
            new DialogueOption(text(dialogueData.cancelText()), "cancel", dialogueaction1, ALWAYS_TRUE, ALWAYS_TRUE)
         ),
         null,
         false
      );
